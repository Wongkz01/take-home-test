...
def github_Authentication():
    ...
    auth_url = 'http://localhost:5000/github/auth'
    response = requests.get(auth_url)

    if response.status_code == 200:
        print("Please log in to your GitHub account to continue.")
        print("After logging in, authorize the application and come back here.")
        print("Press Enter to open the GitHub authentication page...")
        input()

        # Open the GitHub authentication page in the browser
        auth_page_url = response.text.strip()
        import webbrowser
        webbrowser.open_new_tab(auth_page_url)

        # Wait for user to authorize the GitHub account
        print("Once you have authorized the application, please press Enter to continue...")
        input()

        # Retrieve the token from the server
        token_url = 'http://localhost:5000/github/token'
        ...
        if response.status_code == 200:
            ...
            if 'token' in token_data:
                ...
                if token:
                    print("GitHub account authenticated successfully!")

                    # Retrieve the authenticated GitHub username
                    username_url = 'http://localhost:5000/github/username'
                    headers = {'Authorization': f'Bearer {token}'}
                    response = requests.get(username_url, headers=headers)

                    if response.status_code == 200:
                        username = response.json().get('username')
                        print(f"Authenticated GitHub username: {username}")
                        home(token, username)
                    else:
                        print(f"Error retrieving authenticated GitHub username: {response.status_code} - {response.reason}")
                else:
                    print("GitHub account authentication failed.")
            else:
                print("Error: Token not found in the response.")
        else:
            print(f"Error retrieving GitHub token: {response.status_code} - {response.reason}")
    else:
        print(f"Error requesting GitHub authentication: {response.status_code} - {response.reason}")
